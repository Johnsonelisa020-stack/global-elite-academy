import streamlit as st  # Imebadilishwa herufi ndogo hapa
...
# ID yako halisi ya AdSense imewekwa hapa laivu!
...
for idx, paper in enumerate(CURRENT_DATA["Mitihani / Past Papers"]):
    st.markdown(f"""
    <div class='exam-box'>
    ...
    """, unsafe_allow_html=True)
    # Zimeongezewa index kwenye key kuzuia makosa ya upatikanaji
    st.button(f"Download PDF ({paper['jina'][:15]}...)", key=f"btn_{idx}_{paper['jina']}")

st.markdown("---")

# Sehemu ya 3: Chemsha Bongo (Interactive Quiz kwa Session State)
st.header(L["quiz_section"])
quiz = CURRENT_DATA["Quiz"]
st.subheader(quiz["swali"])

# Kuzuia redio kubadilika hovyo pale kitufe kinapobonyezwa
user_ans = st.radio("Chagua jibu lililo sahihi mwanangu:", quiz["majibu"], key=f"quiz_radio_{selected_level}")

if st.button("Hakiki Jibu Lako"):
...
# Sehemu ya 4: AI Intelligent Engine (Iliyoboreshwa isipoteze data)
st.header(L["ai_zone"])
user_query = st.text_input("Uliza jambo lolote kuhusu mitihani au masomo hapa:", placeholder="Mfano: Deadlock inafanyaje kazi kwenye OS?", key="ai_input")
